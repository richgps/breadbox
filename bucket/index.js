...
const GRAPH_PERIOD = 60 * 60 * 3  /* seconds */;
  ...
  if (err) {
    ...
    for (var i = 0; i < awsData.Items.length; i++) {
      ...
      if (typeof item === "object"
          ...
          if (typeof item.payload.M.temp === "object") {
            tempData.push(Number(item.payload.M.temp.N));
            labels.push(getLabel(item));
          }
          if (typeof item.payload.M.humidity === "object") {
            humidityData.push(Number(item.payload.M.humidity.N));
          } else {
            humidityData.push(0);
          }
        }
      ...
      if (err) {
        ...
        if ("state" in payload && "reported" in payload.state) {
          ...
          if (lastAWSData && lastAWSData.Items && lastAWSData.Items.length > 0) {
            ...
            humidity = Number(item.payload.M.humidity.N);
          }
          ...
          if (humidity) $('#humidity').text(humidity.toFixed(1));
